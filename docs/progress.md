...
- **Session: 2026-09-14 (2) — Сверка вскрыла реальную картину эталона:**
  - Диалог сверки показал: лист эталона 202 валидных + 53 конфликта, БД 359,
    отчёт накрыт 1504 из 2427. «Есть в БД, нет на листе: 157» — лист
    перезаписан parseList1ToEtalon (etalon.clear() + запись только валидных).
  - Главное: >5000 записей эталона живут в трёх исторических листах
    («Гугл/Яндекс ру/Яндекс ком — посл эталон разметки») и НИКОГДА не были
    импортированы в БД — функция importHistoricalEtalonsToDb() есть, но не
    запускалась (ранее падала бы из-за бага парсера: бесконечный цикл +
    DEPTH-лимит, исправлены в v1.0.4). До запуска исторического импорта
    сверка с полным эталоном физически неполна.
  - `webhook.py`: fix reconcile — uncovered/mismatches считались как длина
    обрезанного сэмпла (в диалоге показывалось «100» вместо реального числа);
    добавлен invalid_key_rows. Коммит d3e5e93, push выполнен.
  - Пользователю: запустить importHistoricalEtalonsToDb() вручную (Run),
    затем повторная сверка, переразметка 2026-09-14, обновление отчёта.
...
- **Session: 2026-09-08 (3) — Предварительное закрытие v1.0.2:**
  - Решение пользователя: сбор и разметка работают, качество разметки пока
    ...
    после подтверждения качества разметки (LLM vs эталон) и парсинга эталона.
  - CHANGELOG.md: секция [1.0.2] (предварительное закрытие) — Added/Fixed/
    Known limitations по итогам v1.0.2 + hotfix-цепочка f0318d9..aef9226.
  - Tag v1.0.2 НЕ ставится до финального закрытия (явно оговорено:
    последний сбор работал до 09-06, качество не подтверждено).
  - Flush-запись решений — в docs/decisions.md (ADR 2026-09-08).
  - Незакоммиченное в дереве (НЕ из этой сессии): migrate.py,
    opencode.json, .opencode/agents/* — решение за владельцем другой сессии.
  - Гейт финального закрытия: проверка качества разметки заказчиком →
    tag v1.0.2 → завершение сессии флешем (/dream).
...
- **Session: 2026-08-30 — Dynamic etalon v1.0.2:** добавлены изолированный
  ...
  без `geo` и `client_id`; автоматические hooks не используются. Обновлены
  нормативные документы и тесты; targeted и полный набор: `264 passed`.

- **Session: 2026-08-30 — Разрешение конфликта tag:** сохранён legacy
  `v1.0.0` на `3f80fcc`; documentation commit `2c4fdf8` опубликован локально;
  создан annotated `v1.0.1` на `2c4fdf8`. `262/262` теста прошли через
  pre-commit hook. Push не выполнен из-за DNS-недоступности `github.com`.

- **Session: 2026-08-17 — Фиксация SERPlux v1.0:** v1.0 зафиксирована на
  `bdc9a54`; документация закрытия обновлена, проект переведён в
  maintenance-режим, дальнейшая разработка смещается на Overture. Acceptance
  и rollback отделяют подтверждённое от непроверенного.

- **Session: 2026-08-17 — Дедупликация гео в отчёте:** `reporter.py` больше не создаёт две секции `United Kingdom`, когда `Великобритания` и `Лондон` отображаются одним алиасом; добавлен регрессионный тест.
...
- **Session: 2026-07-12 (одиннадцатая) — Онбординг клиента + шаблон листов**
  - [x] **Добавлена функция `initTemplateSheets()` в `apps_script.gs`** — создание 6 листов шаблона (Настройки, Данные, Отчёт, Эталон разметки, Спорные, Лог).
    ...
    - Добавлены константы листов и 6 подфункций (_ensureXxxSheet).
    - Синтаксис проверен: `node --check` ✓
    - Лист1 исключён из структуры.
  - [x] **Переименовано `Лист2` → `Данные`** в exporter.py + тесты + документация.
    - Обновлена константа CACHE_SHEET_NAME.
    - Обновлены комментарии, docstring и тесты test_exporter.py.
    - Обновлены docs/user-guide.md, docs/labeling_canon.md, docs/release-1.0.md.
    - ✅ 224/224 тестов зелёные.
  - [x] **Создан `docs/onboarding-client.md`** — инструкция для заказчика (9 шагов, 360 строк).
    - На русском, без технической кухни (БД, контракты).
    - Poshagovo: Topvisor → копия таблицы → Apps Script → initTemplateSheets() → URL/secret → клиент → запросы → запуск → результат.
    - Раздел FAQ (ошибки и решения).
  - [x] **Обновлён `docs/user-guide.md`:**
    - Добавлен раздел «Добавление нового клиента» со ссылкой на onboarding-client.md.
    - Переписан раздел «Эталон разметки и Спорные» (объяснение структуры листов, автоматического режима, future v2.0).
    - Обновлены описания листа «Отчёт» (динамический размер матрицы по гео и субъектам).
    - Заменены все «Лист2» → «Данные».
  - [x] **Создан `template/SHEETS.md`** — спецификация 6 листов (130 строк).
    - Точные заголовки, структура, типы данных для каждого листа.
    - Раскладка колонок для матрицы (Отчёт).
    - Цветовая разметка (positive/negative/neutral).
    - Механика накопительного режима.
    - Как пересоздать шаблон с нуля.
  - [x] **Обновлён `docs/roadmap-2.0.md`:**
    - Дополнен пункт 1 (Deep-режим): связь с листом «Спорные» в шаблоне.
    - Добавлен новый пункт 1.5 (Изоляция эталона по клиенту): миграция БД, правки кода, тесты, гарантия совместимости.
  - [x] **Обновлены документы проекта:**
    - `docs/CANON.md`: добавлен раздел «Мультиклиентность» (модель, структура листов, динамический размер, кэш разметки, инициализация шаблона).
    - `README.md`: переименовано «Лист2» → «Данные»; добавлена ссылка на docs/onboarding-client.md.
  - [x] **Результат:** 224/224 тестов зелёные. Синтаксис apps_script.gs ✓.
  - Status: Ready for branch + commit
  - Файлы: apps_script.gs (+340 строк), exporter.py (1 строка), tests/test_exporter.py (2 строки), docs/onboarding-client.md (+360 строк), docs/user-guide.md (+50 строк), template/SHEETS.md (+130 строк), docs/roadmap-2.0.md (+30 строк), docs/CANON.md (+40 строк), README.md (2 строки).

- **Session: 2026-07-11 (десятая) — Релизная фиксация v1.0**
  - [x] **Создан `docs/release-1.0.md`** — release notes v1.0 (что вошло, известные ограничения, таблица «заявлено → реализовано»).
  - [x] **Создан `docs/roadmap-2.0.md`** — roadmap v2.0 (7 кандидатов: deep-режим, удаление legacy label_mode, автоматизация эталона, ротация бэкапов, watchdog, уведомления, мониторинг стоимости).
  - [x] **Создан `CHANGELOG.md`** — история изменений (v1.0.0, v0.9.0, v0.8.0, v0.7.0, v0.6.0, v0.5.0, v0.1.0).
  - [x] **Обновлён `README.md`:**
    - Исправлено число тестов (64 → 224).
    - Добавлен полный список эндпоинтов API (11 вместо 3).
    - Добавлены разделы мультиклиентности, накопительного отчёта, эталона разметки.
    - Добавлена строка версии v1.0 и ссылки на release/roadmap.
  - [x] **Переработан `docs/user-guide.md`:**
    - Структура сверху вниз по важности для пользователя: что это → как запустить → где смотреть результат → настройки → эталон → технические детали.
    - Убраны устаревшие разделы (16 колонок, domains/snippets/full).
    - Добавлено описание накопительного режима отчёта.
  - [x] **Тег v1.0.0:** `git tag v1.0.0` на коммите `68a2fb1`.
  - [x] **Результат:** 224/224 тестов зелёные.
  - Status: Ready for commit
  - Коммит: `docs: release v1.0 (release-1.0.md, roadmap-2.0.md, CHANGELOG) + rewrite README/user-guide + tag v1.0.0`

- **Session: 2026-07-11 (девятая) — Накопительный режим отчёта (версии сверху)**
  - [x] **Изменён `reporter.py`: отчёт накапливается сверху вместо перезаписи.**
    - Убран `worksheet.clear()` — лист больше не очищается перед записью.
    - Новый блок отчёта вставляется СВЕРХУ через `insertDimension` (один batch_update запрос).
    - Старые данные сдвигаются вниз вместе с форматированием (заливкой).
    - Хранится максимум 10 версий (`MAX_VERSIONS`), старые обрезаются снизу через `deleteDimension`.
    - Признак начала версии: строка `"Позиции {searcher} на {date}"`.
    - Заливка sentiment применяется к новому блоку после вставки (строки 0..N-1).
  - [x] **Добавлены тесты `TestAccumulativeReport`:**
    - `test_is_version_header` — распознавание заголовков версий.
    - `test_build_report_accumulates_versions` — второй вызов не вызывает `clear()`, делает `insertDimension`.
  - [x] **Обновлён `docs/CANON.md`:** добавлен раздел «Поведение reporter» с описанием накопительного режима.
  - [x] **Результат:** 224/224 тестов зелёные.
  - Status: Ready for commit
  - Коммит: `feat(reporter): accumulate report versions on top (keep last 10, oldest trimmed) + update CANON.md`

- **Session: 2026-07-11 (восьмая) — CANON.md + парсер Лист1 → «Эталон разметки»**
  - [x] **Создан `docs/CANON.md`** — источник истины по геометрии Лист1 и контракту эталона.
    ...
    - Корневая причина: сломанный парсер Лист1 (query=страны, geo=константа).
  - [x] **Создана функция `parseList1ToEtalon()` в `apps_script.gs`** — разовый парсер Лист1 → «Эталон разметки».
    - Запускается вручную через Run в редакторе Apps Script, НЕ в меню onOpen.
    ...
    - Запись в «Эталон разметки»: domain, query, geo, sentiment, source=manual_l1.
  - [x] **Синтаксис apps_script.gs проверен:** `node --check` проходит.
  - [x] **Порядок восстановления эталона:**
    1. `./backup_db.sh`
    2. Запустить `parseList1ToEtalon()` вручную в редакторе Apps Script
    3. Визуально проверить лист «Эталон разметки»: B2=имя субъекта, C2=страна
    4. `DELETE FROM domain_labels WHERE source='manual_l1'`
    5. Запустить `importEtalonToDb()` вручную
    6. Проверить критерии: query=имена субъектов, geo=страны, число записей > 81
  - Status: Ready for commit
  - Коммит: `feat: CANON.md source of truth + fix Лист1 parser (query=subject, geo=country) — restore etalon`
