...
### 2026-08-30 — Push release commit и tag заблокирован DNS

**Проблема:** `git push origin main v1.0.1` не выполнился: окружение не
разрешает hostname `github.com`.

**Влияние:** На GitHub пока не опубликованы `2c4fdf8` и `v1.0.1`; локально
`main` опережает `origin/main` на один commit.

**Что делать:** После восстановления DNS повторить только:
`git push origin main v1.0.1`.

**Статус:** Открыто, инфраструктурный блокер.
