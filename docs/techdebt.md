...
10 или 50 URL в зависимости от настройки Topvisor, а не от Sheets.

**Смягчение (2026-09-06):** подсказка в `SETTINGS_TEMPLATE` теперь явно говорит,
что глубина задаётся в Topvisor, а поле — справочное.

**Что делать:** окончательное UX-решение за заказчиком: либо убрать `depth` из UI
(оставить только в Topvisor), либо оставить как справочный параметр.

**Статус:** Открыто (частично смягчено подсказкой).
...
**Статус:** ✔ Исправлено в 2026-09-06.

Добавлена таблица `providers` (SQLite) и функции `save_provider` / `delete_provider` /
`list_persisted_providers` / `load_providers_into_runtime` в `storage.py`.
Webhook при старте подгружает персистентных провайдеров из БД поверх
встроенного `config.PROVIDERS`; `POST /providers/register`, `PUT/DELETE
/providers/{id}` пишут/удаляют запись в БД. Провайдеры, добавленные через UI,
переживают restart контейнера.
...
`_validate_endpoint()` теперь делает DNS-резолв хоста и блокирует любой адрес
из private/loopback/link-local/reserved/multicast/unspecified диапазонов,
полученный из `socket.getaddrinfo`. Литеральные недопустимые хосты также
отсекаются до резолва.
