...
3. **Имя субъекта** (строка 3) пишется в `sb["pos"]` (pos-колонка), центрировано над блоком.
4. **Заливка sentiment** применяется к ячейке номера позиции (pos-колонка), не к URL и не ко всей строке.
5. **Вертикальная структура:**
   - Строка 1: заголовок `Позиции {провайдер} на {ДД.ММ.ГГГГ}`
   - Строка 2: пустой буфер
   - Строка 3: имя субъекта
   - Строка 4: название гео
   - Строки 5–14: 10 позиций (номер + URL)
...
**Затронутые файлы:**
- `reporter.py`: `_build_subject_layout()` — буфер 3+1; `hdr_row` — имя в `sb["pos"]`.
- `docs/report_layout.md`: создан — канон раскладки.
- `tests/test_reporter.py`: обновлены тесты раскладки, добавлены `TestLayoutBuffers`.
