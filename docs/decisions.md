...
**Решение:**
1. **Кэш по домену:** все функции `get_domain_label()`, `upsert_domain_label()`, `bulk_upsert_domain_labels()` переключены на параметр `domain` вместо `url`.
   - `labeler.py`: добавлена `_extract_domain()` — извлекает домен из URL через `urlparse`.
...
2. **Улучшение промпта LLM:** `_build_prompt()` переписан с few-shot примерами и чёткими критериями.
...
3. **Исправление парсера эталона:** `parseList1ToEtalon()` в Apps Script теперь извлекает домен из URL вместо сохранения полного URL.
...
**Почему:**
- Домен стабилен между прогонами — один и тот же сайт будет найден в кэше независимо от UTM-меток.
- Few-shot примеры значительно улучшают качество разметки LLM (проверено на практике).
- Эталонная разметка теперь корректно матчится с новыми данными через домен.

**Следствия:**
- Существующие записи в domain_labels с полными URL не будут найдены — нужно переимпортировать эталон.
...
- `storage.py`: `get_domain_label()`, `upsert_domain_label()`, `bulk_upsert_domain_labels()` — параметр `domain`
...
## 2026-07-15 — ADR: Переключатель модели и регистрация провайдеров в runtime

**Контекст:** Ранее проект поддерживал только один провайдер (opencode-zen) с одной моделью (qwen3.6-plus), заданной через `OPENCODE_MODEL` в `.env`. Мультипровайдерная цепочка уже была реализована (config.PROVIDERS, priority, enabled), но не было:
1. Возможности выбрать конкретную модель при запуске прогона (без правки .env).
2. Возможности добавить нового провайдера без правки `config.py` и пересборки контейнера.

**Решение:**
1. **Переключатель модели:** параметр `model` в `RunRequest` (`POST /run`) позволяет указать конкретную модель LLM для текущего прогона.
   - Пробрасывается: webhook → main → labeler → _call_provider.
   - `_call_provider` использует `model or provider_cfg["default_model"]` — override имеет приоритет над дефолтом провайдера.
   - Если `model` не указан — используется `default_model` из конфигурации провайдера.
2. **Регистрация провайдеров:** `POST /providers/register` добавляет нового провайдера в `config.PROVIDERS` в runtime.
   - Принимает: provider_id, enabled, priority, default_model, models, endpoint, api_key_env_var.
   - Возвращает 201 Created при успехе, 409 Conflict если provider_id уже существует.
   - Провайдеры хранятся в памяти (dict) — при перезапуске контейнера нужно добавить провайдер в `config.py` или `.env`.
3. **`config.register_provider()`:** функция для программной регистрации, валидирует обязательные ключи.
4. **`.env.example`:** добавлены placeholder'ы для второго провайдера (SECONDARY_PROVIDER_*).

**Почему:**
- Гибкость: пользователь может переключать модели без перезапуска контейнера.
- Расширяемость: новый провайдер добавляется через API, не через правку кода.
- Обратная совместимость: `model=None` сохраняет старое поведение (default_model провайдера).
- Runtime-регистрация — временное решение; для персистентности провайдеров нужно хранить их в БД (future).
...
## 2026-07-11 — ADR: Накопительный режим отчёта (версии сверху, макс. 10)

**Контекст:** Ранее `build_report` делал `worksheet.clear()` + `worksheet.update` — каждый прогон стирал лист и писал заново. История отчётов не сохранялась.

**Решение:**
1. Убран `worksheet.clear()` — лист больше не очищается.
2. Новый блок отчёта вставляется СВЕРХУ через `insertDimension` (один batch_update запрос).
3. Старые данные сдвигаются вниз вместе с форматированием (заливкой).
4. Хранится максимум 10 версий (`MAX_VERSIONS`), старые обрезаются снизу через `deleteDimension`.
5. Признак начала версии: строка `"Позиции {searcher} на {date}"`.
6. Заливка sentiment применяется к новому блоку после вставки (строки 0..N-1).

**Почему:**
- История отчётов нужна для сравнения версий и аудита.
- Накопительный режим позволяет видеть динамику позиций во времени.
- Ограничение в 10 версий предотвращает разрастание листа.
- `insertDimension` сдвигает форматирование автоматически — не нужно пересчитывать координаты старых заливок.

**Статус:** Принято и реализовано

**Затронутые файлы:**
- `reporter.py`: добавлены `_is_version_header()`, `_count_versions()`, `_trim_old_versions()`; изменён `build_report()`.
- `tests/test_reporter.py`: добавлен `TestAccumulativeReport` (2 теста).
- `docs/CANON.md`: добавлен раздел «Поведение reporter».
- `docs/progress.md`: обновлён.

---

## 2026-07-11 — ADR: CANON.md — источник истины по геометрии и контракту эталона

**Контекст:** Лист «Эталон разметки» был сгенерён сломанным парсером Лист1: в query попали страны вместо имён субъектов, в geo — константа "United Kingdom" вместо реальных стран. Из-за этого ~420 размеченных ячеек схлопнулись в ~81 битую запись, и кэш domain_labels не работает.

**Решение:**
1. Создан `docs/CANON.md` — источник истины по геометрии Лист1 и контракту эталона.
   - Геометрия: имя субъекта в правой колонке (C, H, K…), гео в левой (B, G, J…), буферы 3+1.
   - Контракт: query=имя субъекта (lowercase), geo=страна, domain из URL, sentiment по цвету.
   - Карта граблей: что проверено и НЕ трогать.
2. Создана функция `parseList1ToEtalon()` в `apps_script.gs` — разовый парсер Лист1 → «Эталон разметки».
   - Запускается вручную через Run, НЕ в меню onOpen.
   - Определяет субъектов по строке 3 (имя в правой колонке).
   - Для каждого субъекта проходит гео-блоки (гео в левой колонке, под ним depth номеров).
   - Для каждой ячейки номера: цвет заливки → sentiment, URL → domain, имя субъекта → query, гео → geo.
   - Строки без цвета/URL → лист «Спорные».

**Почему:**
- CANON.md — первое, что читается в начале новой сессии, чтобы не повторять ошибки.
- Парсер должен строго следовать геометрии Лист1, а не додумывать.
- Разовая функция (не в меню) — после восстановления эталона не нужна.

**Статус:** Принято и реализовано

**Затронутые файлы:**
- `docs/CANON.md`: создан — источник истины.
- `apps_script.gs`: добавлена `parseList1ToEtalon()`, `_colorToSentiment()`, `_extractDomain()`.
- `docs/progress.md`: обновлён.

---

## 2026-07-11 — ADR: Каноническая раскладка отчёта (Лист1-эталон)

**Контекст:** Раскладка листа «Отчёт» должна точно повторять эталон «Лист1» (Google Sheets). Ранее `_build_subject_layout()` использовал симметричные буферы (1 колонка между всеми субъектами), что не соответствовало эталону.

**Решение:**
1. **Асимметричные буферы:**
   - После первого субъекта: 3 пустые колонки (D, E, F).
   - Перед каждым последующим субъектом (≥2): 1 пустая колонка.
2. **Формула колонок (0-indexed):**
   - S1: pos=1 (B), url=2 (C)
   - S2: pos=6 (G), url=7 (H)
   - S3: pos=9 (J), url=10 (K)
   - S4: pos=12 (M), url=13 (N)
   - Для N≥2: `pos = 6 + (N-2)*3`, `url = pos + 1`
3. **Имя субъекта** (строка 3) пишется в `sb["url"]` (правая колонка), крупным шрифтом.
4. **Гео** (строка 4) пишется в `sb["pos"]` (левая колонка).
5. **Заливка sentiment** применяется к ячейке номера позиции (pos-колонка), не к URL и не ко всей строке.
6. **Вертикальная структура:**
   - Строка 1: заголовок `Позиции {провайдер} на {ДД.ММ.ГГГГ}`
   - Строка 2: пустой буфер
   - Строка 3: имя субъекта (в url-колонке)
   - Строка 4: название гео (в pos-колонке)
   - Строки 5–14: 10 позиций (номер в pos-колонке с заливкой, URL в url-колонке)
   - Строка 15: пустой буфер
   - Повтор для следующего гео.

**Почему:**
- Соответствие эталону: заказчик сверяет визуальное представление с Лист1.
- Асимметрия буферов (3+1) — историческое решение из оригинального шаблона, сохранено для совместимости.
- Заливка на pos-ячейке (не на URL) — визуально понятнее: цвет привязан к номеру позиции.
- Имя субъекта в url-колонке (правая) — соответствует эталону Лист1.

**Статус:** Принято и реализовано

**Затронутые файлы:**
- `reporter.py`: `_build_subject_layout()` — буфер 3+1; `hdr_row` — имя в `sb["url"]`.
- `docs/report_layout.md`: создан — канон раскладки.
- `tests/test_reporter.py`: обновлены тесты раскладки, добавлены `TestLayoutBuffers`, `TestSentimentFillCoordinates`.

**История исправлений:**
- 2026-07-11: первоначальная реализация (имя в pos-колонке) — ошибочно.
- 2026-07-11: фикс — имя перенесено в url-колонку (правая), гео в pos-колонку (левая). Коммит `25582d3`.

---

## 2026-07-11 — ADR: Демо-данные вынесены из exporter.py

**Контекст:** В `exporter.py` были захардкожены `test_rows_1`/`test_rows_2` (демо-строки с `example.com`) и `__main__` блок, который экспортировал их в Google Sheets. При прямом запуске `exporter.py` демо-данные затирали реальные данные на листе «Лист2».

**Решение:**
1. Удалены `test_rows_1`, `test_rows_2` и `__main__` блок из `exporter.py`.
2. Создан `scripts/demo_export.py` — изолированный демо-прогон, запускаемый только при `SERPLUX_DEMO=1`.
3. Экспортер пишет ТОЛЬКО реальные данные из БД (positions + domain_labels за дату прогона).

**Почему:**
- Безопасность: демо-данные не должны попадать в боевые листы.
- Чистота кода: exporter.py — production-модуль, не содержит тестовых заглушек.
- Grep-проверка: `grep -rn "example\|test_rows" exporter.py reporter.py` → ноль совпадений.

**Статус:** Принято и реализовано

**Затронутые файлы:**
- `exporter.py`: удалён `__main__` блок.
- `scripts/demo_export.py`: создан (за `SERPLUX_DEMO=1`).
- `tests/test_exporter.py`: добавлен `TestNoDemoDataInExport`.

---

## 2026-07-11 — ADR: verify.sh — warning не роняет проверку

**Контекст:** `verify.sh` при обнаружении warning (шаг 4/6 — логи) увеличивал `WARNINGS`, но не увеличивал `CHECKS_PASSED`. Это приводило к `Checks: 5/6 passed` + `Verification failed` + `exit 1`, даже если все проверки прошли, но были warning'и.

**Решение:**
1. `log_check("warn")` теперь увеличивает `CHECKS_PASSED` (warning = passed + пометка).
2. Grep логов (шаг 4/6) исключает строки-метрики (`_error=0`, `fallback_neutral=0`, `=N`) — это INFO-статистика об отсутствии ошибок, не реальные сбои.
3. `exit 1` только при реальном fail (не passed).

**Почему:**
- Warning ≠ failure: warning означает «обрати внимание», а не «проверка провалена».
- Метрики вида `provider_error=0` — это好消息 (хорошая новость), а не ошибка.
- `verify.sh` должен возвращать `exit 0` при `6/6 passed`, даже если были warning'и.

**Статус:** Принято и реализовано

**Затронутые файлы:**
- `verify.sh`: `log_check("warn")` увеличивает `CHECKS_PASSED`; grep исключает метрики.
...
2. Раскладка колонок вычисляется динамически функцией `_build_subject_layout(queries)` (см. ADR 2026-07-11 о каноне буферов 3+1)
3. Порядок географических регионов берётся из `client.regions_map`, fallback на глобальный `GEO_ORDER`
4. `build_report()` теперь принимает параметры `client_id` и `db_path`, загружает профиль из БД
5. `config.SUBJECT_BLOCKS` и `config.COLS` помечены как `_DEPRECATED_*` (сохранены для обратной совместимости)

**Почему:**
- Фабричное решение: код не содержит данных клиента, профиль хранится в БД
- Масштабируемость: клиент с 2 субъектами → 8 колонок, с 4 субъектами → 14 колонок, без правки кода
...
**Статус:** Принято и реализовано (раскладка уточнена в ADR 2026-07-11)
...
- `tests/test_reporter.py`: тесты для 2, 4, 7 субъектов + TestLayoutBuffers
- `tests/test_config.py`: обновлены для использования `_DEPRECATED_*`
- `docs/report_layout.md`: канон раскладки (создан 2026-07-11)
...
## 2026-07-10 — ADR: Разделение кэша и отчёта в Google Sheets + канон разметки
**Контекст:** В лист «Отчёт» попадал кэш выдачи (date, searcher, query, geo, URL,
snippet, label) вместе с матрицей-отчётом. Это засоряло отчёт и мешало
визуальной проверке. Одновременно требовалось зафиксировать единый канон
разметки, чтобы устаревшие режимы `domains`/`snippets`/`full` не возвращались.

**Решение:**
1. **Кэш и отчёт разделены:**
   - `exporter.py` пишет кэш выдачи на лист **«Лист2»** с полной очисткой
     перед записью.
   - `reporter.py` пишет только отчёт за текущую дату на лист **«Отчёт»**,
     также с полной очисткой перед записью.
2. **Канон разметки зафиксирован в `docs/labeling_canon.md`:**
   - Допустимые `sentiment`: `positive`, `negative`, `neutral`.
   - Источники `source`: `manual_l1` > `snippet`/`page`; `manual_l1` не
     перезаписывается автоматикой.
   - Режимы `label_mode` для новых прогонов: только `auto` и `deep`;
     `domains`/`snippets`/`full` — legacy.
   - `neutral` — легитимный маркер неуверенности.

**Почему:**
- Отчёт должен содержать только сводную матрицу, а кэш — диагностический
  слой для ручной проверки разметки.
- Единый канон исключает неоднозначность в допустимых значениях и режимах.

**Что НЕ делаем:**
- Не удаляем legacy-режимы `domains`/`snippets`/`full` из БД и `storage.LABEL_MODES`
  прямо сейчас — это требует миграции существующих меток (см. `docs/techdebt.md`).
- Не меняем приоритет `upsert_domain_label` — он уже соответствует канону.

**Затронутые файлы:** `exporter.py`, `reporter.py`, `storage.py`,
`tests/test_exporter.py`, `docs/labeling_canon.md`, `docs/contracts.md`,
`docs/techdebt.md`, `docs/progress.md`.
