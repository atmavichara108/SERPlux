...
**Что проверяет:**
- **Python tests:** `pytest -v` — все unit/integration тесты (172 тестов)
- **Shellcheck:** статический анализ `verify.sh`, `backup_db.sh`, `deploy.sh`
  и других `.sh` файлов. Ловит `set -e` + арифметика, `grep` без совпадений,
  неинициализированные переменные и др.
- Python 3.11 окружение
- зависимости из `requirements.txt` + `requirements-dev.txt`

**Результат:** ✓ или ✗ в GitHub UI (блокирует merge, если тесты или
shellcheck упали)

**Не проверяет:** деплой, доступ к серверу, боевые данные, реальное
выполнение `verify.sh` в Docker-контейнере. Подробнее о границах локального
тестирования — в `docs/infra-testing.md`.
